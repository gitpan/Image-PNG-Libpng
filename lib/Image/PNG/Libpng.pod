...
C<$row_pointers> should be set up something like the following:

    RETVAL = newSV (0);
    xv_setref_pv (RETVAL, "Image::PNG::Libpng::row_pointers", row_pointers);

It's extracted using
...
you can find each instance L<in the file C<perl-libpng.c> in the top directory of the distribution|http://cpansearch.perl.org/src/BKB/Image-PNG-Libpng-0.24/perl-libpng.c> in the macro
C<UNUSED_ZERO_ARG>.
