...
C<day>, C<hour>, C<minute>, and C<second>. If the argument is omitted,
the time is set to the current time. If any of year, month, day, hour,
minute or second is omitted, these are set to zero. PNG tIME chunks do
not contain a time zone. "Universal Time (UTC) should be specified
...
This function corresponds to C<png_get_sCCP>
...
This function corresponds to C<png_set_sCCP>
...
These functions are used to access parts of the C<$png> object itself.
...
the PNG image. This is then freed by Image::PNG::Libpng using Perl's
Safefree when C<$png> is destroyed. Because the freeing uses
C<Safefree>, it's better to use Newx to allocate the memory rather
than malloc.
...
you can find each instance L<in the file C<perl-libpng.c> in the top directory of the distribution|http://cpansearch.perl.org/src/BKB/Image-PNG-Libpng-0.26/perl-libpng.c> in the macro
C<UNUSED_ZERO_ARG>.
