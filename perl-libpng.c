...
#define UNSUPPORTED(block)						\
    perl_png_warn (png, #block " chunk not supported in this libpng");
...
/* Return an undefined value. This is also going to raise an error if
   the user wants it to, but the code is not here yet. */
...
perl_png_destroy (perl_libpng_t * png)
{
    ...
    /* Allow whatever copied rows to be freed. */
    if (png->copied_rows) {
	SvREFCNT_dec ((SV *) png->copied_rows);
    }
}
...
#ifdef PNG_iTXt_SUPPORTED
        if (is_itxt) {
            HASH_FETCH_PV (chunk, language_tag);
            HASH_FETCH_PV (chunk, translated_key);
...
static AV *
perl_png_colours_to_av (png_colorp colours, int n_colours)
{
    int i;
    AV * perl_colours;
    perl_colours = newAV ();
...
static SV * perl_png_get_tRNS (perl_libpng_t * png)
{
    ...
    colour_type = png_get_color_type (pngi);
...
static void perl_png_set_sPLT (perl_libpng_t * png, AV * sPLT_entries)
{
	...
	if (SvOK (* sv) && SvROK (* sv) && SvTYPE (SvRV (* sv)) == SVt_PVHV) {
	    ...
	    perl_png_warn (png, "Not a hash reference at position %d", i);
	    continue;
	}
	...
	if (SvOK (* sv) && SvROK (* sv) && SvTYPE (SvRV (* sv)) == SVt_PVAV) {
	    ...
	    perl_png_warn (png, "Could not get entries at position %d", i);
	    continue;
	}
	...
	for (j = 0; j < nentries; j++) {
		...
		perl_png_warn (png, "Could not get entry %d", j);
		continue;
	    }
	    MESSAGE ("Copying entry %d", j);
	    e = entry->entries + j;
#line 1662 "perl-libpng.c"
...
static SV * perl_png_get_sBIT (perl_libpng_t * png)
{
    if (VALID (sBIT)) {
        HV * sig_bit;
        png_color_8p colours;
	png_uint_32 status;
        sig_bit = newHV ();
	status = png_get_sBIT (pngi, & colours);
	if (status != PNG_INFO_sBIT) {
	    /* error */
	}
#line 1781 "perl-libpng.c"
        
	HASH_STORE_IV_MEMBER (sig_bit, red, colours);
	
	HASH_STORE_IV_MEMBER (sig_bit, green, colours);
	
	HASH_STORE_IV_MEMBER (sig_bit, blue, colours);
	
	HASH_STORE_IV_MEMBER (sig_bit, gray, colours);
	
	HASH_STORE_IV_MEMBER (sig_bit, alpha, colours);
	
#line 1768 "perl-libpng.c.tmpl"
...
static void perl_png_set_rows (perl_libpng_t * png, AV * rows)
{
    ...
    png->copied_rows = rows;
    SvREFCNT_inc ((SV *) rows);
}
